...
async def handle_voice(message: Message, bot):
    processing_msg = await message.answer("🎙️ Ovoz qabul qilindi...")
    ...
    try:
        ...
        # Faylni xotirada (BytesIO) saqlaymiz, tempfile va ffmpeg shart emas
        voice_bin = await bot.download_file(file.file_path)
        voice_bytes = voice_bin.read()

        await processing_msg.edit_text("🤖 Gemini tahlil qilmoqda...")

        # 2. Gemini API (Live emas, Content Generation usuli barqarorroq)
        client = genai.Client(api_key=GEMINI_API_KEY)
        
        # OGG formatini to'g'ridan-to'g'ri yuboramiz
        response = client.models.generate_content(
            model=MODEL, # Hozirgi barqaror multimodal model
            ...
            contents=[
                types.Part.from_bytes(
                    data=voice_bytes,
                    mime_type="audio/ogg" # Telegram ogg/opus yuboradi
                ),
                "Ushbu audioni tahlil qiling va o'zbek tilida javob bering."
            ]
        )
        ...
        # 3. Javobni yuborish
        if response.text:
            await message.answer(f"🤖 <b>Gemini:</b> {response.text}")
        else:
            await message.answer("❌ Tushunarsiz audio yoki javob bo'sh.")
